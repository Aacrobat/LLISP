...
class presony(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.input_images[str(ratio)[0:3]][ind] is None:
            ...
            self.gt_images[ind] = np.expand_dims(pack_raw(gt_raw), axis=0)
...
class f_SonyDataset(Dataset):
    ...
    def __init__(self, input_dir, gt_dir, ps=512):
        ...
        self.fns = glob.glob(input_dir + '0*.ARW') 
        self.ids = [int(os.path.basename(fn)[0:5]) for fn in self.fns] 
        self.sids = [int(os.path.basename(fn)[6:8]) for fn in self.fns] 
    ...
    def __getitem__(self, ind):
        id = self.ids[ind]
        sid = self.sids[ind]
        in_files = glob.glob(self.input_dir + '%05d_%02d*.ARW' % (id,sid))
        in_path = in_files[np.random.random_integers(0, len(in_files) - 1)] 
        in_fn = os.path.basename(in_path)
        ...
        return input_patch, gt_patch, id, sid, ratio


class f_SonyTestDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        return input_full, scale_full, gt_full, id, sid, ratio
